    ...
    stages {
      stage('Pull Docker Image from DockerHub') {
            steps{
               ...
               image = docker.image('dmitrykzn/sborshik1')
               image.pull() 
                 }
               }
            }
      ...
      stage('Run Docker container from image') {
          agent { 
            docker {
              image 'registry.hub.docker.com/dmitrykzn/sborshik1'
        ...
        stages {
          ...
          stage ('Make docker image and tag it') {
            steps {
              ...
              sh 'cp Dockerfile ./target/ && cd target/ && ls && ls && docker build -t war_with_apache .'
          ...
          stage ('Push Docker image to GitHub') {
            steps {
              script {
               docker.withRegistry('https://registry.hub.docker.com', 'docker-hub-credentials') {
               image = docker.image('dmitrykzn/war_with_apache:v1')
               image.push()
               }
              }
            }
